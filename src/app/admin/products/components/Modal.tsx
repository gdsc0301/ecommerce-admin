...
import { addProductModalImage, closeProductModal, openProductModal, removeProductModalImage, setProductField, setProductModalNewImage } from "../slices/Modal";
...
const ProductModal = (props: {products: Product[] | undefined, mutator: KeyedMutator<Product[]>, loading: boolean}) => {
  ...
  const handleFieldChange = (e: ChangeEvent<HTMLInputElement>) => {
    appDispatch(setProductField({field: e.target.name as keyof typeof product, value: e.target.value}));
  }
  
  return (
    <Modal isOpen={isOpen} onOpenChange={handleOpenChange} size="5xl">
      <ModalContent>
        ...
        <ModalFooter>
          <Button onClick={close} color="success" variant="shadow">{__(product ? 'Save' : 'Add')}</Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
  )
